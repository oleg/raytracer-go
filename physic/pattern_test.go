...
func Test_creating_stripe_pattern(t *testing.T) {
	pattern := StripePattern{geom.White, geom.Black, NoTransformation}
...
func Test_stripes_with_object_transformation(t *testing.T) {
	object := Transformable{geom.Scaling(2, 2, 2)}
	pattern := StripePattern{geom.White, geom.Black, NoTransformation}
...
func Test_stripes_with_pattern_transformation(t *testing.T) {
	pattern := StripePattern{geom.White, geom.Black, Transformable{geom.Scaling(2, 2, 2)}}

	c := PatternAtShape(pattern, NoTransformation, geom.Point{X: 1.5, Y: 0, Z: 0})
...
func Test_stripes_with_both_object_and_pattern_transformation(t *testing.T) {
	object := Transformable{geom.Scaling(2, 2, 2)}
	pattern := StripePattern{geom.White, geom.Black, Transformable{geom.Translation(0.5, 0, 0)}}
...
func Test_gradient_linearly_interpolates_between_colors(t *testing.T) {
	pattern := GradientPattern{geom.White, geom.Black, NoTransformation}
...
func Test_ring_should_extend_in_both_x_and_z(t *testing.T) {
	pattern := RingPattern{geom.White, geom.Black, NoTransformation}
...
func Test_checkers_should_repeat_in_x_y_z(t *testing.T) {
	pattern := CheckersPattern{geom.White, geom.Black, NoTransformation}
