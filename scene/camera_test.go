...
func Test_constructing_ray_with_camera(t *testing.T) {
	tests := []struct {
		...
		{"Constructing a ray through the center of the canvas",
			...
			100, 50,
			shapes.Ray{geom.Point{X: 0, Y: 0, Z: 0}, geom.Vector{X: 0, Y: 0, Z: -1}},
		},
		{"Constructing a ray through a corner of the canvas",
			NewCameraDefault(201, 101, math.Pi/2),
			0, 0,
			shapes.Ray{geom.Point{X: 0, Y: 0, Z: 0}, geom.Vector{X: 0.66519, Y: 0.33259, Z: -0.66851}},
		},
		{"Constructing a ray when the camera is transformed",
			NewCamera(201, 101, math.Pi/2, geom.RotationY(math.Pi/4).Multiply(geom.Translation(0, -2, 5))),
			100, 50,
			shapes.Ray{geom.Point{X: 0, Y: 2, Z: -5}, geom.Vector{X: math.Sqrt2 / 2, Y: 0, Z: -math.Sqrt2 / 2}},
		},
	}
