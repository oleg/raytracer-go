...
func Test_hit_intersections(t *testing.T) {
	...
	tests := []struct {
		...
		expectedFound        bool
		expectedIntersection ddddf.Inter
	}{
		{"all intersections have positive t",
			ddddf.Inters{ddddf.Inter{2, s}, ddddf.Inter{1, s}}, true, ddddf.Inter{1, s}},
		{"some intersections have negative t",
			ddddf.Inters{ddddf.Inter{1, s}, ddddf.Inter{-1, s}}, true, ddddf.Inter{1, s}},
		{"all intersections have negative t",
			ddddf.Inters{ddddf.Inter{-1, s}, ddddf.Inter{-2, s}}, false, ddddf.Inter{}},
		{"is always the lowest non negative intersection",
			ddddf.Inters{ddddf.Inter{5, s}, ddddf.Inter{7, s}, ddddf.Inter{-3, s}, ddddf.Inter{2, s}}, true, ddddf.Inter{2, s}},
	}
...
func Test_precomputing_reflection_vector(t *testing.T) {
	shape := ddddf.MakePlane()
	ray := ddddf.Ray{geom.Point{X: 0, Y: 1, Z: -1}, geom.Vector{X: 0, Y: -math.Sqrt2 / 2, Z: math.Sqrt2 / 2}}
	i := ddddf.Inter{math.Sqrt2, shape}

	comps := PrepareComputations(i, ray, ddddf.Inters{i})
...
func Test_finding_n1_and_n2_at_various_intersections(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			comps := PrepareComputations(xs[test.index], r, xs)
