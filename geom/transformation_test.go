...
func Test_multiply_point_by_translation_matrix(t *testing.T) {
	...
	assert.Equal(t, Point{X: 2, Y: 1, Z: 7}, r)
}
...
func Test_multiply_point_by_inverse_of_translation_matrix(t *testing.T) {
	...
	assert.Equal(t, Point{X: -8, Y: 7, Z: 3}, r)
}
...
func Test_scaling_matrix_applied_to_point(t *testing.T) {
	...
	assert.Equal(t, Point{X: -8, Y: 18, Z: 32}, r)
}
...
func Test_scaling_matrix_applied_to_vector(t *testing.T) {
	...
	assert.Equal(t, Vector{X: -8, Y: 18, Z: 32}, r)
}
...
func Test_multiplying_inverse_of_scaling_matrix(t *testing.T) {
	...
	assert.Equal(t, Vector{X: -2, Y: 2, Z: 2}, r)
}
...
func Test_reflection_is_scaling_by_negative_value(t *testing.T) {
	...
	assert.Equal(t, Point{X: -2, Y: 3, Z: 4}, r)
}
...
func Test_rotating_point_around_x_axis(t *testing.T) {
	tests := []struct {
		...
		{"half quarter", math.Pi / 4, Point{X: 0, Y: math.Sqrt2 / 2, Z: math.Sqrt2 / 2}},
		{"full quarter", math.Pi / 2, Point{X: 0, Y: 0, Z: 1}},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			tr := RotationX(test.rotation)

			r := tr.MultiplyPoint(Point{X: 0, Y: 1, Z: 0})
...
func Test_rotating_point_around_y_axis(t *testing.T) {
	tests := []struct {
		...
		{"half quarter", math.Pi / 4, Point{X: math.Sqrt2 / 2, Y: 0, Z: math.Sqrt2 / 2}},
		{"full quarter", math.Pi / 2, Point{X: 1, Y: 0, Z: 0}},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			tr := RotationY(test.rotation)

			r := tr.MultiplyPoint(Point{X: 0, Y: 0, Z: 1})
...
func Test_rotating_point_around_z_axis(t *testing.T) {
	tests := []struct {
		...
		{"half quarter", math.Pi / 4, Point{X: -math.Sqrt2 / 2, Y: math.Sqrt2 / 2, Z: 0}},
		{"full quarter", math.Pi / 2, Point{X: -1, Y: 0, Z: 0}},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
...
func Test_inverse_of_x_rotation_rotates_in_opposite_direction(t *testing.T) {
	...
	expected := Point{X: 0, Y: math.Sqrt2 / 2, Z: -math.Sqrt2 / 2}
...
func Test_shearing_transformation(t *testing.T) {
	tests := []struct {
		...
		{"x in proportion to y", Shearing(1, 0, 0, 0, 0, 0), Point{X: 5, Y: 3, Z: 4}},
		{"x in proportion to z", Shearing(0, 1, 0, 0, 0, 0), Point{X: 6, Y: 3, Z: 4}},
		{"y in proportion to x", Shearing(0, 0, 1, 0, 0, 0), Point{X: 2, Y: 5, Z: 4}},
		{"y in proportion to z", Shearing(0, 0, 0, 1, 0, 0), Point{X: 2, Y: 7, Z: 4}},
		{"z in proportion to x", Shearing(0, 0, 0, 0, 1, 0), Point{X: 2, Y: 3, Z: 6}},
		{"z in proportion to y", Shearing(0, 0, 0, 0, 0, 1), Point{X: 2, Y: 3, Z: 7}},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
...
func Test_individual_transformations_are_applied_in_sequence(t *testing.T) {
	...
	AssertPointEqualInDelta(t, Point{X: 1, Y: -1, Z: 0}, p2)
	AssertPointEqualInDelta(t, Point{X: 5, Y: -5, Z: 0}, p3)
	AssertPointEqualInDelta(t, Point{X: 15, Y: 0, Z: 7}, p4)
}
...
func Test_chained_transformations_must_be_applied_in_reverse_order(t *testing.T) {
	...
	assert.Equal(t, Point{X: 15, Y: 0, Z: 7}, r)
}
